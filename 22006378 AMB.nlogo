...
;Setting agent characteristics and the world
to setup_world
  clear-turtles
  reset-ticks
  set student_id 22006378
  set student_name "Anup Gururaj Rao Kulkarni"
...
; For my reference only. not for marking. Thank you
;  create-turtles initially_infected
;  [
;    set color red
;    setxy random max-pycor random-xcor
;    set corrupt true
...
to recurrview
  set current_lime_population count turtles with
  [
    group = "lime turtle"
  ]
  set current_cyan_population count turtles with
  [
    group = "cyan turtle"
  ]

  set count_turt count turtles
  if count_turt > 0
  [
  set total_infected_percentage 100 * (count turtles with [
      color = red ]) / count_turt
  ]

  if current_lime_population > 0
  [
    set lime_infected_percentage (count turtles with [ color = red and group = "lime turtle" ]) * 100 / current_lime_population
  ]

  if current_cyan_population > 0
  [
    set cyan_infected_percentage (count turtles with [ color = red and group = "cyan turtle" ])* 100 / current_cyan_population
  ]

  if count_turt > 0
  [
    set total_antibodies_percentage (count turtles with [ color = black ])* 100 / count_turt
  ]

  if current_lime_population > 0
  [ set lime_antibodies_percentage (count turtles with [ color = black and group = "lime turtle" ]) * 100 / current_lime_population
  ]

  if current_cyan_population > 0 [ set cyan_antibodies_percentage (count turtles with [color = black and group = "cyan turtle" ]) * 100 / current_cyan_population
  ]
  set total_active_cases count turtles with [ corrupt ]   ;edit
end
...
to socialMovement
  ifelse count turtles-on patch-ahead 1 = 0 [ make_isolate
  ]

  [ rt one-of (range -45 46)
...
to turtle_movement
  rt one-of (range -20 21)
  ( ifelse social_distancing [socialMovement] self_isolation [ make_isolate ]
    travel_restrictions [ restrict_travel ] [ fd 0.2 ] )
end
...
to inject
  ask other turtles-here with [ not corrupt ]
  [ infect ]
end
...
to run_model
  ask turtles [
    iteration
    turtle_movement
    if color = red or color = orange [ live-leave ]
    if corrupt [ inject ]
  ]
...
"" ""
PENS
"cyan population" 1.0 0 -11221820 true "" "plot cyan_population"
"lime population" 1.0 0 -13840069 true "" "plot lime_population"
...
"immune" 1.0 0 -16710398 true "" "plot total_antibodies_percentage"
